...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mQueue = VolleySingleton.getInstance(this@MainActivity).getRequestQueue()
        ...
        val request : JsonObjectRequest = JsonObjectRequest(url, {
            try {
                ...
                for (i in 0 until jsonArray.length()) {
                    ...
                    val firstName : String = employee.getString("first_name")
                    val lastName : String = employee.getString("last_name")
                    val email : String = employee.getString("email")

                    mTextViewResult.append("Name : ${firstName} ${lastName} \nEmail Address : ${email} \n\n")
                }
